...
**Last scan:** 2026-02-20T07:54:55+00:00  
...
|---------|----------|:-----------:|:----:|
| `CON-DVO-002` | `packages/showcase/src/database/connection.ts` | ![fail](https://img.shields.io/badge/-FAIL-e05d44?style=flat-square) | ![semantic](https://img.shields.io/badge/-semantic-purple?style=flat-square) |
| `CON-DVO-002` | `packages/showcase/src/types/Auth.ts` | ![fail](https://img.shields.io/badge/-FAIL-e05d44?style=flat-square) | ![semantic](https://img.shields.io/badge/-semantic-purple?style=flat-square) |
| `CON-PFM-001` | `packages/showcase/src/database/connection.ts` | ![fail](https://img.shields.io/badge/-FAIL-e05d44?style=flat-square) | ![semantic](https://img.shields.io/badge/-semantic-purple?style=flat-square) |
| `CON-PFM-001` | `packages/showcase/src/database/queries.ts` | ![fail](https://img.shields.io/badge/-FAIL-e05d44?style=flat-square) | ![semantic](https://img.shields.io/badge/-semantic-purple?style=flat-square) |
| `CON-PFM-001` | `packages/showcase/src/database/types.ts` | ![fail](https://img.shields.io/badge/-FAIL-e05d44?style=flat-square) | ![semantic](https://img.shields.io/badge/-semantic-purple?style=flat-square) |
| `CON-PFM-009` | `packages/showcase/src/data/mockCustomers.ts` | ![fail](https://img.shields.io/badge/-FAIL-e05d44?style=flat-square) | ![semantic](https://img.shields.io/badge/-semantic-purple?style=flat-square) |
| `CON-DVO-002` | `packages/showcase/src/database/queries.ts` | ![warn](https://img.shields.io/badge/-WARN-dfb317?style=flat-square) | ![semantic](https://img.shields.io/badge/-semantic-purple?style=flat-square) |
| `CON-DVO-002` | `packages/showcase/src/database/types.ts` | ![warn](https://img.shields.io/badge/-WARN-dfb317?style=flat-square) | ![semantic](https://img.shields.io/badge/-semantic-purple?style=flat-square) |
| `CON-DVO-002` | `packages/showcase/src/types/ui-kit.d.ts` | ![warn](https://img.shields.io/badge/-WARN-dfb317?style=flat-square) | ![semantic](https://img.shields.io/badge/-semantic-purple?style=flat-square) |
| `CON-PFM-001` | `packages/showcase/src/types/Customer.ts` | ![warn](https://img.shields.io/badge/-WARN-dfb317?style=flat-square) | ![semantic](https://img.shields.io/badge/-semantic-purple?style=flat-square) |
| `CON-PFM-009` | `packages/showcase/src/database/types.ts` | ![warn](https://img.shields.io/badge/-WARN-dfb317?style=flat-square) | ![semantic](https://img.shields.io/badge/-semantic-purple?style=flat-square) |
| `CON-PFM-010` | `package.json` | ![warn](https://img.shields.io/badge/-WARN-dfb317?style=flat-square) | ![semantic](https://img.shields.io/badge/-semantic-purple?style=flat-square) |
| `CON-PFM-010` | `packages/showcase/package.json` | ![warn](https://img.shields.io/badge/-WARN-dfb317?style=flat-square) | ![semantic](https://img.shields.io/badge/-semantic-purple?style=flat-square) |
| `CON-PFM-010` | `packages/ui-kit/package.json` | ![warn](https://img.shields.io/badge/-WARN-dfb317?style=flat-square) | ![semantic](https://img.shields.io/badge/-semantic-purple?style=flat-square) |

<details>
<summary><strong>Evidence details (14 findings)</strong></summary>

#### ![fail](https://img.shields.io/badge/-FAIL-e05d44?style=flat-square) `CON-DVO-002` — `packages/showcase/src/database/connection.ts`
...
#### ![warn](https://img.shields.io/badge/-WARN-dfb317?style=flat-square) `CON-DVO-002` — `packages/showcase/src/database/queries.ts`
...
#### ![warn](https://img.shields.io/badge/-WARN-dfb317?style=flat-square) `CON-DVO-002` — `packages/showcase/src/database/types.ts`

> User role type definition only includes 'admin' | 'user' (lines 22, 41, 71, 79) - overly simplistic role model that may not support proper separation of duties for security-critical operations
...
#### ![fail](https://img.shields.io/badge/-FAIL-e05d44?style=flat-square) `CON-DVO-002` — `packages/showcase/src/types/Auth.ts`

> Lines 2-6: User interface only defines two roles ('admin' | 'user') which is insufficient for separation of duties. No distinction between security-critical operations like payment creation vs approval, no separate database roles (app, migration, audit, admin), and no MFA enforcement in auth types.
...
#### ![warn](https://img.shields.io/badge/-WARN-dfb317?style=flat-square) `CON-DVO-002` — `packages/showcase/src/types/ui-kit.d.ts`

> Line 4-8: User interface defines only basic roles 'admin' | 'user' without proper separation of duties for security-critical operations. This overly broad role definition could lead to insufficient access control granularity.
...
#### ![fail](https://img.shields.io/badge/-FAIL-e05d44?style=flat-square) `CON-PFM-001` — `packages/showcase/src/database/connection.ts`

> Lines 11-12: Single shared database file path 'DB_PATH = join(__dirname, '../../database.sqlite')' and global singleton database connection 'let db: Database | null = null;'. Lines 21-45: initializeDatabase() creates a single shared database instance. Lines 50-55: getDatabase() returns the same shared database connection for all tenants. All query functions (runQuery, getAllQuery, getQuery) operate on the same shared database without any tenant isolation.
...
#### ![fail](https://img.shields.io/badge/-FAIL-e05d44?style=flat-square) `CON-PFM-001` — `packages/showcase/src/database/queries.ts`

> All SQL queries lack tenant_id scoping. Examples: 'SELECT * FROM customers WHERE id = ?' (line 40), 'SELECT COUNT(*) as count FROM customers' (line 28), 'INSERT INTO customers...' (line 48), 'DELETE FROM customers WHERE id = ?' (line 97), 'SELECT * FROM users WHERE id = ?' (line 142). No tenant isolation is implemented - queries access global tables without tenant filtering.
...
#### ![fail](https://img.shields.io/badge/-FAIL-e05d44?style=flat-square) `CON-PFM-001` — `packages/showcase/src/database/types.ts`

> Database entity interfaces Customer and User lack tenant_id fields. Lines 7-18 (Customer interface) and lines 20-26 (User interface) define database entities without tenant isolation fields. The rule requires 'All queries are tenant-scoped' and 'Foreign keys include tenant_id' but these core entities have no tenant identification.
...
#### ![warn](https://img.shields.io/badge/-WARN-dfb317?style=flat-square) `CON-PFM-001` — `packages/showcase/src/types/Customer.ts`
...
#### ![fail](https://img.shields.io/badge/-FAIL-e05d44?style=flat-square) `CON-PFM-009` — `packages/showcase/src/data/mockCustomers.ts`
...
#### ![warn](https://img.shields.io/badge/-WARN-dfb317?style=flat-square) `CON-PFM-009` — `packages/showcase/src/database/types.ts`

> PaginationOptions uses 'limit' instead of 'pageSize' (line 67), and 'sortBy/sortOrder' instead of 'sort/order' (lines 68-69). PaginatedResult uses 'limit' instead of 'pageSize' (lines 74, 76). No API response format types following { success: true, data, correlationId } pattern.
...
#### ![warn](https://img.shields.io/badge/-WARN-dfb317?style=flat-square) `CON-PFM-010` — `package.json`
...
#### ![warn](https://img.shields.io/badge/-WARN-dfb317?style=flat-square) `CON-PFM-010` — `packages/showcase/package.json`
...
#### ![warn](https://img.shields.io/badge/-WARN-dfb317?style=flat-square) `CON-PFM-010` — `packages/ui-kit/package.json`
