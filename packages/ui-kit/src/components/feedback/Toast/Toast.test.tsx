...
describe('Toast', () => {
    ...
    it('should apply correct variant styling', () => {
        ...
        expect(toastElement).toHaveClass('bg-[hsl(var(--success))]');

        // Test error variant
        rerender(<Toast toast={{ ...mockToast, variant: 'error' }} />);
        toastElement = screen.getByRole('alert');
        expect(toastElement).toHaveClass('bg-[hsl(var(--error))]');

        // Test warning variant
        rerender(<Toast toast={{ ...mockToast, variant: 'warning' }} />);
        toastElement = screen.getByRole('alert');
        expect(toastElement).toHaveClass('bg-[hsl(var(--warning))]');

        // Test info variant
        rerender(<Toast toast={{ ...mockToast, variant: 'info' }} />);
        toastElement = screen.getByRole('alert');
        expect(toastElement).toHaveClass('bg-[hsl(var(--info))]');
    });
