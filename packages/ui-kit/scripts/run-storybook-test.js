...
for (let i = 0; i < args.length; i++) {
  if (args[i] === "--pattern" && i + 1 < args.length) {
    ...
    storyPattern = args[i + 1];
    i++; // Skip next argument as it's the story pattern value
  }
}
...
async function startStorybook() {
  ...
  const storybookProcess = spawn(
    "storybook",
    ["dev", "--ci", "--port", availablePort.toString()],
    {
      shell: true,
  ...
  async function tryDetectPortFromLsof(rejectFn) {
    try {
      ...
      rejectFn(
        new Error(
          `Timeout waiting for Storybook to start (${STORYBOOK_TIMEOUT_MS / 1000} seconds)`,
        ),
      );
    } catch (err) {
      rejectFn(
        new Error(
          `Timeout waiting for Storybook to start. Failed to detect from lsof: ${err.message}`,
        ),
      );
    }
  }
  ...
  const portDetectionPromise = new Promise((resolve) => {
    ...
    const checkForPort = (output) => {
      ...
      for (const pattern of patterns) {
        ...
        if (match && match[1]) {
          ...
          if (detectedPort > 0) {
            console.log(
              `Detected Storybook running on port: ${detectedPort} (matched pattern: ${pattern})`,
            );
            port = detectedPort;
            portDetected = true;
            resolve(port);
      ...
      if (output.includes(`${availablePort}`) && !portDetected) {
        console.log(
          `Detected Storybook likely running on requested port: ${availablePort}`,
        );
        port = availablePort;
        portDetected = true;
        resolve(port);
    ...
    setTimeout(() => {
      if (!portDetected) {
        console.log(
          `No port detected after 30s. Assuming port ${availablePort} is being used...`,
        );
        port = availablePort;
        portDetected = true;
        resolve(port);
  ...
  try {
    ...
    try {
      ...
      const { stdout: netstatOutput } = await execAsync(
        "netstat -tulpn 2>/dev/null | grep node",
      );
  ...
  console.log(
    `Waiting additional ${READY_WAIT_MS / 1000} seconds for Storybook to fully initialize...`,
  );
...
async function runTests(port) {
  ...
  if (storyPattern) {
    console.log(`Filtering stories by pattern: ${storyPattern}`);
  }
  ...
  try {
    ...
    try {
      ...
      if (storyPattern) {
        const filteredStories = Object.keys(storyData.entries).filter((id) =>
          id.toLowerCase().includes(storyPattern.toLowerCase()),
        );
        console.log(
          `Filtered to ${filteredStories.length} stories matching "${storyPattern}":`,
        );
        filteredStories.slice(0, 10).forEach((id) => {
          console.log(`- Story ID: ${id}`);
        });
      } else {
        Object.keys(storyData.entries)
          .slice(0, 10)
          .forEach((id) => {
            console.log(`- Story ID: ${id}`);
          });
    ...
    const outputFile = join(process.cwd(), "tmp", "a11y-test-output.log");
    console.log(`Test output will be written to: ${outputFile}`);
    ...
    // Add Jest arguments after -- separator for filtering
    if (testPattern || storyPattern) {
      testArgs.push("--");

      if (testPattern) {
        testArgs.push("--testNamePattern", testPattern);
      }

      if (storyPattern) {
        // Use Jest's testPathPattern for file filtering
        testArgs.push("--testPathPattern", storyPattern);
      }
    }

    console.log(`Running test-storybook with args: ${testArgs.join(" ")}`);

    // Run the tests against the detected port with additional flags
    const testProcess = spawn("test-storybook", testArgs, {
      shell: true,
      stdio: ["pipe", "pipe", "pipe"],
    });
    ...
    return new Promise((resolve, reject) => {
      testProcess.on("close", (code) => {
        ...
        const filename =
          testPattern || storyPattern
            ? `a11y-test-output-${testPattern || storyPattern}-${timestamp}.log`
            : `a11y-test-output-${timestamp}.log`;
        const specificOutputFile = join(process.cwd(), "tmp", filename);

        const fullOutput = `=== TEST CONFIGURATION ===\nTest Pattern: ${testPattern || "all"}\nStory Pattern: ${storyPattern || "all"}\nTimestamp: ${new Date().toISOString()}\n\n=== STDOUT ===\n${outputBuffer}\n\n=== STDERR ===\n${errorBuffer}\n\n=== EXIT CODE ===\n${code}`;

        try {
          // Ensure tmp directory exists
          execSync("mkdir -p tmp");

          writeFileSync(specificOutputFile, fullOutput);
          console.log(`\nTest output written to: ${specificOutputFile}`);
        } catch (writeError) {
